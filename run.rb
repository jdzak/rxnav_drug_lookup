  ...
  def role_concepts_in_definition(json)
    ...
    if full_concept = json['fullConcept']
      if group_roles = full_concept['groupRoles']
        if roles = group_roles && group_roles.map.map{ |gr| gr['role'] }
          if roles
            roles.flatten!
...
puts "moa_tree: #{moa_tree.map{ |c| c[:name] }}"
# puts "role_concepts: #{role_concepts}"
# moa_tree << role_concepts.map do |rc|
#   resource.find_parent_concepts_by_nui(rc[:nui])
# end
